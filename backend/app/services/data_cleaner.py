    ...
    try:
        ...
        if retention_count > 0:
            ...
            for sid in server_ids:
                successful_backups = (
                    db.query(JenkinsBackup)
                    .filter(
                        JenkinsBackup.server_id == sid,
                        JenkinsBackup.status == "SUCCESS",
                    )
                    .order_by(JenkinsBackup.backup_time.desc())
                    .all()
                )
                if len(successful_backups) > retention_count:
                    expired_backups = successful_backups[retention_count:]
                    expired_paths = [
                        backup.zip_path
                    ...
                    for path in expired_paths:
                        try:
                            ...
                            logger.warning(
                                f"Failed to remove expired backup file {path}: {error}"
                            )
